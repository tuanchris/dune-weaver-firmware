...
#include "WebServer.h"
#include <lwip/sockets.h> // DW fork: SO_LINGER on accepted clients
#include "FS.h"
...
// that fd), abort whatever request is mid-flight, then recycle the listener —
// closing it makes lwIP abort every connection still queued on it, and
// SO_REUSEADDR (set in WiFiServer::begin) lets us re-listen immediately on
// the same port. Handler registrations survive.
void WebServer::restartListener() {
  WiFiClient pending = _server.available();
  if (pending) {
    struct linger l = { 1, 0 };
    pending.setSocketOption(SOL_SOCKET, SO_LINGER, &l, sizeof(l));
    pending.stop();
  }
  _currentClient.stop();
  close();
...
void WebServer::_handleRequest() {
  ...
  // DW fork: liveness stamp for the self-heal watchdog. A 404 counts — any
  // completed request proves the server is still serving.
  _lastHandledMs = millis();
  _currentUri = "";
}
