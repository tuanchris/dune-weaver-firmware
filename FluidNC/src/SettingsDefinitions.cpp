...
void make_settings() {
    ...
    start_message =
        new StringSetting("Message issued at startup", EXTENDED, WG, NULL, "Start/Message", "Grbl \\V [dune-weaver \\B (\\R) \\H]", 0, 45);
    ...
    // holdOk: both of these are consulted at HOME time, and resuming a paused
    // job never re-homes -- so writing them during a pause cannot touch the job
    // in flight.  (Aligning the orientation is a different matter: that JOGS
    // via $Sand/Goto, which requires Idle -- see SandApi::goTo.)
    homing_mode = holdOk(new EnumSetting("Sand homing mode", EXTENDED, WG, NULL, "Sand/HomingMode", HomingSensor, &homingModeOptions));
    ...
    theta_offset = holdOk(new IntSetting("Theta zero offset (deg)", EXTENDED, WG, NULL, "Sand/ThetaOffset", 0, -360, 360));
    ...
    // holdOk: locking/unlocking the API touches nothing the paused job uses,
    // and the app's Security card sits in the same Settings screen as the
    // playlist/LED ones.
    sand_password = holdOk(new StringSetting("API password (empty = open)", EXTENDED, WG, NULL, "Sand/Password", "", 0, 32));
